    ...
    const fetchData = async () => {
      try {
        const [movieRes, creditsRes] = await Promise.all([
          fetch(`https://api.themoviedb.org/3/movie/${movieId}?api_key=${API_KEY}&language=ko-KR`, { headers: HEADERS }),
          fetch(`https://api.themoviedb.org/3/movie/${movieId}/credits?api_key=${API_KEY}&language=ko-KR`, { headers: HEADERS }),
        ])

        if (!movieRes.ok) throw new Error(`영화 정보를 불러오지 못했습니다. (${movieRes.status})`)
        if (!creditsRes.ok) throw new Error(`출연진 정보를 불러오지 못했습니다. (${creditsRes.status})`)
        ...
        <div className="flex-1 flex flex-col gap-3">
          ...
          <div className="flex flex-wrap gap-2 items-center">
            <span className="flex items-center gap-1 bg-yellow-500/20 text-yellow-400 text-sm font-semibold px-3 py-1 rounded-full">
              ...
              <span className="text-gray-400 font-normal text-xs">({movie.vote_count.toLocaleString()}명)</span>
            </span>
      ...
      {credits.crew.length > 0 && (
        <section className="mb-10">
          ...
          <div className="flex flex-col gap-3">
            {['Director', 'Producer', 'Screenplay', 'Original Music Composer', 'Director of Photography']
              .flatMap((job) =>
                credits.crew
                  .filter((c) => c.job === job)
              ...
              .map((crew) => (
                <div key={`${crew.id}-${crew.job}`} className="flex items-center gap-3 bg-gray-800/60 rounded-xl px-4 py-3">
